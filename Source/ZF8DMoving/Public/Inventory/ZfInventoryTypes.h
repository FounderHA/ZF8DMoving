...
struct ZF8DMOVING_API FZfEquipmentList : public FFastArraySerializer
{
    ...
    UPROPERTY(NotReplicated)
    TObjectPtr<class UZfEquipmentComponent> OwnerComponent = nullptr;

    // Replicação delta do equipamento
    bool NetDeltaSerialize(FNetDeltaSerializeInfo& DeltaParams)
    {
        return FFastArraySerializer::FastArrayDeltaSerialize<FZfEquipmentSlotEntry, FZfEquipmentList>(
